...
Native Claude records and `ps` may render the same process start in UTC and local
time. Agenthail compares those values as instants after validating the PID,
socket path, socket owner and socket type. A different start instant remains a
