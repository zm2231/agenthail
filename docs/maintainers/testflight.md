...
- `resend-invitation`: request one invitation email for an existing tester already assigned to the configured group. An Apple `ALREADY_ACCEPTED` response is a successful no-op. Apple accepting a send request does not confirm inbox delivery or acceptance. An uncertain result is not retried automatically. See [Apple beta tester invitations](https://developer.apple.com/documentation/appstoreconnectapi/beta-tester-invitations).
