...
Use **+** in Conversations to start work with a configured Claude, Codex or Notion runtime. Choose a recent Mac workspace or enter an existing directory for Claude/Codex, optionally choose a model, and write the first instruction. Runtime permission defaults remain in effect unless you explicitly change the Claude permission setting; approvals may require attention on the Mac. If creation cannot be confirmed, check the agent catalog on your Mac before retrying. If the server returns a created session with uncertain first-instruction delivery, the app opens that session and labels the uncertainty.
