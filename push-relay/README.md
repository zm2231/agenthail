...
The release workflow tests the relay before building native artifacts. Its final publication job uploads the validated iPhone build, waits for TestFlight to mark the exact version and build number valid, deploys the reviewed relay with the locked Wrangler version, verifies `/health` against the release tag and protocol 2, and only then publishes the draft GitHub release. To roll back, check out the last known-good tag, deploy its `push-relay` directory with the same secrets, then confirm `/health` before rerunning or restoring a native release.
