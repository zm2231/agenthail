...
cleanup() {
	if [ -x "$NEW_BIN/agenthail" ]; then
		HOME="$TEST_HOME" PATH="$FAKE_BIN:/opt/homebrew/bin:/usr/bin:/bin" "$NEW_BIN/agenthail" daemon stop >/dev/null 2>&1 || true
	elif [ -x "$OLD_BIN/agenthail" ]; then
		HOME="$TEST_HOME" PATH="$FAKE_BIN:/opt/homebrew/bin:/usr/bin:/bin" "$OLD_BIN/agenthail" daemon stop >/dev/null 2>&1 || true
	fi
	if [ -x "$CUSTOM_BIN/agenthail" ]; then
		HOME="$CUSTOM_HOME" PATH="$FAKE_BIN:/opt/homebrew/bin:/usr/bin:/bin" "$CUSTOM_BIN/agenthail" daemon stop >/dev/null 2>&1 || true
	fi
...
grep -Fq 'stopping running daemon for upgrade' "$TMP/custom-in-place.log"
grep -Fq 'restarting daemon with the upgraded binary' "$TMP/custom-in-place.log"
...
HOME="$SUPERVISED_HOME" PATH="$FAKE_BIN:/opt/homebrew/bin:/usr/bin:/bin" "$SUPERVISED_BIN/agenthail" daemon install >/dev/null
